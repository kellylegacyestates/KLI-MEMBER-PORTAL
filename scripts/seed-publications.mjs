...
const seededAt = Timestamp.fromDate(new Date("2026-08-15T00:00:00.000Z"));
...
const publication = {
  ...
  subtitle: null,
  series: { name: "KLI Research Paper Series", number: "2026-01" },
  ...
  distribution: {
    ssrn: { status: "preliminary_upload", abstractId: "7284922", url: null },
    zenodo: { status: "draft", doi: "10.5281/zenodo.21955550", url: null },
    website: { status: "planned", url: null },
    journal: {
  ...
  cta: { label: "Study the underlying doctrine inside KLI", url: null },
  createdAt: seededAt,
  updatedAt: seededAt,
};

const reference = db.collection("publications").doc(publication.id);
const existing = await reference.get();

if (existing.exists) {
  console.log(`${publication.id} already exists; no record was overwritten.`);
} else {
  await reference.create(publication);
  console.log(`Created publications/${publication.id}.`);
}
